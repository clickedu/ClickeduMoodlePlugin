...
class clickedu_invalid_response extends moodle_exception {
    ...
    function __construct($path, array $params, $response) {
        if (!is_string($response)) {
            $response = json_encode($response);
        }

        $info =  $path . ' => ' . $response;
        add_to_log(SITEID, 'clickedu', 'error', '', $info);
        ...
        parent::__construct($string, 'local_clickedu', '' , $a, $info);
    }
}
...
class clickedu_moodle_webservice_setting extends admin_setting_configcheckbox {
    ...
    public function get_setting() {
        return clickedu_moodle_webservice_configured() ? $this->yes : $this->no;
    }
...
function clickedu_check_token() {
    ...
    } elseif (isset($result['error']) or !isset($result['status'])) {
        return CLICKEDU_TOKEN_INVALID;
...
function clickedu_get_course_enrolments($courseid) {
    ...
    foreach ($records as $record) {
        ...
        } elseif ($record->roleid == $studentroleid) {
...
function clickedu_get_courses() {
    ...
    foreach (clickedu_webservice_query('/moodle/cgaps') as $course) {
        ...
        $id_cgap = $course['id_cgap'];

        $courseid = $DB->get_field('course', 'id', array('idnumber' => $id_cgap));
    ...
    foreach (clickedu_webservice_query('/moodle/cgaps.users', $params) as $course) {
        ...
        $courses[$id_cgap]->newstudents = array_keys($users);

        if ($courses[$id_cgap]->oldstudents != $courses[$id_cgap]->newstudents) {
            $courses[$id_cgap]->needssync = true;
        }
    }
...
function clickedu_login($username, $password) {
    ...
    if (isset($result['status']) and $result['status'] == 'acceptat') {
        add_to_log(SITEID, 'clickedu', 'login', '', $username);
        return true;
    }
...
function clickedu_sync_contents(stdClass $course, array $contents, progress_bar $progress) {
    ...
    add_to_log($course->id, 'clickedu', 'sync_contents');
}
...
function clickedu_sync_courses(array $courses, progress_bar $progress) {
    ...
    $info = count($courses) . ' courses synced';
    add_to_log(SITEID, 'clickedu', 'sync_courses', '', $info);
...
function clickedu_create_users(array $users, progress_bar $progress) {
    ...
    $info = count($users) . ' new users';
    add_to_log(SITEID, 'clickedu', 'sync_users', '', $info);
